...
def test_stats_command_empty_library() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "Traceback" not in result.stderr
        assert "No tracks yet" in result.stdout or "0 tracks" in result.stdout or "Library Statistics" in result.stdout
...
def test_quickplay_command_no_files() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert result.returncode in [0, 1]
        assert "Traceback" not in result.stderr
        assert "No audio files found" in result.stdout or "Quick Play Mode" in result.stdout
...
def test_list_batches_command() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "Traceback" not in result.stderr
        assert ("test_mix_001.csv" in result.stdout or "Batches" in result.stdout or "mix_" in result.stdout)
...
def test_download_command_nonexistent_batch() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        result = subprocess.run([
            get_python_command(), "bang_tunes.py", "download", "nonexistent_batch.csv"
        ], capture_output=True, text=True,
           cwd=Path(__file__).parent.parent,
           env={"BANGTUNES_ROOT": tmpdir})
